...
class AttnDecoderRNN(nn.Module):
    ...
    def forward(self, input, hidden, encoder_output, encoder_outputs):
        ...
        # encoder_output: encoder final cell state 
        # encoder_outputs: encoder cell state for every input word
        embedded = self.embedding(input).view(1, 1, -1)
...
class PtrAttnDecoderRNN(nn.Module):
    def __init__(self, hidden_size, vocab_size, max_length, n_layers=1, dropout_p=0.1):
        super(PtrAttnDecoderRNN, self).__init__()
        self.hidden_size = hidden_size
        self.vocab_size = vocab_size
        # self.output_size = vocab_size+max_length
        ...
        self.ptr = nn.Linear(self.hidden_size * 3, 1)

    def calculateFinalProb(self, p_gen, p_vocab, attn_weights, input_variable):
        # p_vocab = p_gen*p_vocab
        p_vocab = torch.mul(p_vocab, p_gen.repeat(1, self.vocab_size))
        
        # p_copy = (1-p_gen)*p_copy
        ...
        p_extended_copy = p_extended_copy.scatter(1, input_extended, p_copy)
        output = torch.add(p_extended_vocab, p_extended_copy)
        return output

    def forward(self, input, hidden, encoder_output, encoder_outputs, input_variable):
        # input: previous word
        # hidden: encoder final hidden state
        # encoder_output: encoder final cell state 
        # encoder_outputs: encoder cell state for every input word
        ...
        non_ptr_output = F.log_softmax(self.out(output[0]))

        p_vocab = F.softmax(self.out(output[0]))
        
        p_gen = F.sigmoid(self.ptr(torch.cat((embedded[0], attn_applied[0], output[0]), 1)))
        
        ptr_output = self.calculateFinalProb(p_gen, p_vocab, attn_weights, input_variable)
        
        return ptr_output, non_ptr_output, hidden, attn_weights
...
def train(input_variable, target_variable, encoder, decoder, encoder_optimizer, decoder_optimizer, criterion, max_length, hidden_size):
    ...
    if use_teacher_forcing:
        ...
        for di in range(target_length):
            decoder_ptr_output, decoder_output, decoder_hidden, decoder_attention = decoder(
                decoder_input, decoder_hidden, encoder_output, encoder_outputs, input_variable)
            topv, topi = decoder_ptr_output.data.topk(1)
            ni = topi[0][0]
            if ni < decoder.vocab_size:
                loss += criterion(decoder_ptr_output[0], target_variable[di])
            else:
...
def evaluate(lang, encoder, decoder, sentence, max_length):
    ...
    for di in range(max_length):
        ...
        topv, topi = decoder_output.data.topk(1)
        ni = topi[0][0]
        if ptr_ni < lang.n_words:
            # if ni != ptr_ni:
            #     print('Ptr network copy')
            ni = ptr_ni
        if ni == EOS_token:
...
def main(args):
    ...
    attn_decoder1 = PtrAttnDecoderRNN(args.hidden_size, lang.n_words, args.max_sent_len, 1, dropout_p=0.1)
    if use_cuda:

...
sys_comb_file = open('/corpora/yahoo_answers/final_data/Entertainment_Music/test/answers.Entertainment_Music.sents.batch040.sys_comb_by_bleu.formal', 'w')
sys_comb_with_modelname_file = open('/corpora/yahoo_answers/final_data/Entertainment_Music/test/answers.Entertainment_Music.sents.batch040.sys_comb_by_bleu.formal.withmodelname', 'w')

model_sentences = [None]*6
model_sentence_bleu = [None]*6
for i in range(6):
	model_sentences[i] = open(model_files[i], 'r').readlines()
	model_sentence_bleu[i] = [float(score.strip('\n')) for score in open(model_files[i]+'.sent_bleu_scores', 'r').readlines()]
...
for j in range(len(model_sentences[0])):
	...
	for i in range(6):
		...
		scores.append(model_sentence_bleu[i][j])
